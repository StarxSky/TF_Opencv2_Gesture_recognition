...
def train_model(train_data):
    ...
    checkpoint_filepath = "D:\\aiFile\\model_save\\gesture_recognition_model\\"
    callbacks = [
        # 保存模型的回调函数
    ...
    # 模型训练
    network.load_weights('D:\\aiFile\\model_save\\gesture_recognition_model\\gestureModel_one.h5')
    print("载入已训练权重成功")
    network.fit(train_data, epochs=5, callbacks=callbacks)  # 因为是dataset数据集是个元组自带标签所以不用分x和y了
    # network.evaluate(test_data)
    network.save_weights('D:\\aiFile\\model_save\\gesture_recognition_model\\gestureModel_one.h5')
    print("保存模型权重成功")
    tf.saved_model.save(network, 'D:\\aiFile\\model_save\\gesture_recognition_model\\gestureModel_one')
    print("保存模型成功")
    return network
...
if __name__ == "__main__":
    ...
    test_dir = 'D:\\code\\PYTHON\\gesture_recognition\\testdata'
